angular.module('ddStatePicker', ['ui.select', 'ngSanitize'])
    .directive('ddStatePicker', function ($http) {
        var template = '<ui-select ng-model="$parent.ngModel" reset-search-input="false" required\
                     on-select="onSelect($item, $model)" ui-select-open-on="{{openOn}}">\
                        ...
                        <ui-select-choices repeat="state in (states | filter: $select.search) track by $index">\
                            {{state.name}}\
                        </ui-select-choices>\
                    </ui-select>';
